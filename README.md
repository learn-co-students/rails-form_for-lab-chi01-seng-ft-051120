...
- **Models** - Create a `SchoolClass` model, along with its associated database table. The `SchoolClass` table should have the columns: `title:string` and `room_number:integer`. The `Student` model and database table have already been created for you. [X]

- **Controllers** - Create controller files for `school_classes` and `students`. [X] Build out the `new`[X], `create`[X], `show`[X], `edit`[X], and `update`[X] actions in each of the controllers.
